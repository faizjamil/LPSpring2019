...
{  
  public static void main(String[] args) throws 
                                             IOException
  {
    System.out.println("S3 compiler written by Faizan Jamil");
    ...
    //debug token trace via command line	 
	if (args[0].equalsIgnoreCase("-debug_token_manager")) 
	{
	  debug = true;
	  inFileName = args[args.length - 1] + ".s";
	  outFileName = args[args.length - 1] + ".a";
	} else {
	  debug = false;
	  inFileName = args[0] + ".s";
	  outFileName = args[0] + ".a";
	}
    ...
    outFile.println("; from S3 compiler written by Faizan Jamil");
...
class S3TokenMgr implements S3Constants
{
  ...
  public Token getNextToken()
  {
    ...
    if (currentChar == EOF)
    {
     ...
     // if string spans a line, output ERROR
     /*else
     if ((currentChar == '\n') || (currentChar == '\r'))
                   token.kind = ERROR;*/
    ...
    if (currentChar == '"')
    {
      ...
      // change the length of buffer`
      // dont want to append newline or backslash to buffer
      // save buffer as String in token.image
      if (currentChar =='"')
      {
        buffer.append(currentChar);
...
class S3Parser implements S3Constants
{
  ...
  private void assignmentStatement()
  {
    ...
    consume(ID);
    st.enter(t.image);
    cg.emitInstruction("pc", t.image); //xxxxxxxx
    consume(ASSIGN);
    assignmentTail();
  ...
  private void assignmentTail()
  {
    ...
    {
      //apply seconds assignmentTail production
      expr();
      //cg.emitInstruction("stav");
      consume(SEMICOLON);
    }
  ...
  //-----------------------------------------
  //printlnStatement selection set: {"println", "{",
  private void printlnStatement()
  {
    ...
    if (currentToken.kind != RIGHTPAREN)
      printArg();
    //print out string followed by new line
  ...
  private void printArg()
  {
    ...
    switch(currentToken.kind)
    {
      ...
      default:
        expr();
        cg.emitInstruction("dout"); //xxxxxxxxxxxxxxxxxx
    }
  }
  private void printStatement()
  {
    ...
    printArg();
    //cg.emitInstruction("pc", "'\\n'");   //xxxxxxxxxxxxxx
    //cg.emitInstruction("aout");         //xxxxxxxxxxxxxxx
    consume(RIGHTPAREN);
    consume(SEMICOLON);
  }
  ...
  private void readintStatement()
  {
    consume(READINT);
    consume(LEFTPAREN);
    //<ID> part of production, insert assembly here
    Token t = currentToken;
    consume(ID);
    st.enter(t.image);
    cg.emitInstruction("pc", t.image);
  ...
  private void factor()
  {
    ...
    switch(currentToken.kind)
    {
      ...
      case MINUS:
        consume(MINUS);
        switch(currentToken.kind) //xxxxxxxxxxxxxxxx
        {
          case UNSIGNED:
            t = currentToken;
            consume(UNSIGNED);
          ...
          case LEFTPAREN:
            consume(LEFTPAREN);
            expr();
            consume(RIGHTPAREN);  // xxxxxxxxxxxxxx
            cg.emitInstruction("neg");
            break;
          case PLUS:
            consume(PLUS);
